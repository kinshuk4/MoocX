...
class UserCode:
    def __init__(self):
        ...
        #process noise
        pos_noise_std = 0.008 # 0.005
        yaw_noise_std = 0.025 # 0.005
        self.Q = np.array([
        ...
        # measurement noise
        z_pos_noise_std = 0.25 # 0.005
        z_yaw_noise_std = 0.025 # 0.08 # 0.03
        self.R = np.array([
    ...
    def calculateKalmanGain(self, sigma_p, H, R):
        ...
        '''
        return np.dot(np.dot(sigma_p, H.T), np.linalg.inv(np.dot(H, \
            np.dot(sigma_p, H.T)) + R))

    def correctState(self, K, x_predicted, z, z_predicted):
        '''
        corrects the current state prediction using Kalman gain, the measurement and the
        ...
        return x_predicted + np.dot(K, z - z_predicted)
    ...
    def predictMeasurement(self, x, marker_position_world, marker_yaw_world):
        ...
        return np.array([[z_predicted.translation[0],
            z_predicted.translation[1],
            z_predicted.yaw()]]).T

    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
        '''
        calculates the 3x3 Jacobian matrix of the predictMeasurement(...) function using the
        current state and the marker position and orientation in world coordinates

        :param x - current state 3x1 vector
        :param marker_position_world - x and y position of the marker in world coordinates 2x1
            vector
        ...
        H[0,0] = -math.cos(psi)
        H[0,1] = -math.sin(psi)
        H[0,2] = -math.sin(psi) * (x_m[0] - x[0]) + math.cos(psi) * (x_m[1] - x[1])
        H[1,0] =  math.sin(psi)
        H[1,1] = -math.cos(psi)
        H[1,2] = -math.cos(psi) * (x_m[0] - x[0]) -math.sin(psi) * (x_m[1] - x[1])
        H[2,0] =  0.0
        H[2,1] =  0.0
        H[2,2] = -1.0
    ...
    def state_callback(self, t, dt, linear_velocity, yaw_velocity):
        ...
        :param linear_velocity - x and y velocity in local quadrotor coordinate frame
            (independet of roll and pitch)
    ...
    def measurement_callback(self, marker_position_world, marker_yaw_world, marker_position_relative, marker_yaw_relative):
        '''
        called when a new marker measurement arrives max 30Hz, marker measurements are only
        available if the quadrotor is sufficiently close to a marker

        :param marker_position_world - x and y position of the marker in world coordinates 2x1
            vector
        :param marker_yaw_world - orientation of the marker in world coordinates
        :param marker_position_relative - x and y position of the marker relative to the
            quadrotor 2x1 vector
        :param marker_yaw_relative - orientation of the marker relative to the quadrotor
        '''
        z = np.array([[marker_position_relative[0],
            marker_position_relative[1],
            marker_yaw_relative]]).T
