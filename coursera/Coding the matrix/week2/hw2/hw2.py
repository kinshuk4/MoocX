...
def vec_select(veclist, k): 
    ...
    True
    '''
    return [ v for v in veclist if v.f.setdefault(k, 0) == 0]
...
def vec_sum(veclist, D): 
    ...
    True
    '''
    k = {d: sum([ v.f.setdefault(d, 0) for v in veclist]) for d in D }
    return Vec(D, k)
...
def vec_select_sum(veclist, k, D): 
    ...
    True
    '''
    return vec_sum([v for v in veclist if v.f.setdefault(k, 0) == 0], D)
...
def scale_vecs(vecdict):
    ...
    True
    '''
    return [Vec(v.D, {d: (v.f.setdefault(d, 0)/k) for d in v.D}) for k, v in vecdict.items()]
