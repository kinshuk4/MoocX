...
import org.mk.training.jdbc.ExceptionSorter;
...
public class OracleExceptionSorter implements ExceptionSorter, Serializable {
	...
	public boolean isExceptionFatal(final SQLException e) {
		...
		if ((error_code == 28) // session has been killed
				...
				|| (error_code == 1090) // shutdown in progress - connection is
										// not permitted
				|| (error_code == 1092) // ORACLE instance terminated.
		...
		if ((error_code < 20000 || error_code >= 21000)
				&& ((error_text.indexOf("SOCKET") > -1) // for control socket
														// error
						|| (error_text
								.indexOf("CONNECTION HAS ALREADY BEEN CLOSED") > -1) || (error_text
