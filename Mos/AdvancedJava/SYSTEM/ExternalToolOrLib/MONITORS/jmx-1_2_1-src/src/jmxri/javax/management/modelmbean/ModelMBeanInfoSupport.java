	 ...
	 implements ModelMBeanInfo, java.io.Serializable
{
	...
	*/
	 ...
	 * displayName=ClassName, persistPolicy=never,log=F, visiblity=1
	 *
	 ...
	 * descriptor null, a default descriptor will be constructed.
	 * The default descriptor is:
	 * name=className, descriptorType=mbean, displayName=className,
	 * persistPolicy=never, log=F, visiblity=1.  If the
	...
	throws MBeanException, RuntimeOperationsException {
		...
		{
			retList = new Descriptor[1];
			retList[0] = modelMBeanDescriptor;
		}
		if (inDescriptorType.equalsIgnoreCase(ATTR))
		{
		...
		} else
		{
			throw new RuntimeOperationsException(new IllegalArgumentException("Descriptor Type is invalid"),
												 ("Exception occured trying find the descriptors of the MBean"));
		}
		if (tracing())
		{
	...
	throws MBeanException, RuntimeOperationsException {
		...
		{ // throw RuntimeOperationsException - invalid descriptor
			throw new RuntimeOperationsException(new IllegalArgumentException("Descriptor list is invalid"),
												 ("Exception occured trying set the descriptors of the MBeanInfo"));
		}
	...
	throws MBeanException, RuntimeOperationsException {
		...
		if ((inDescriptorType == null) || (inDescriptorType == ""))
		{
			...
			if (inDescriptorType == null)
			{ // thow RuntimeOperationsException - invalid descriptor type
				throw new RuntimeOperationsException(new IllegalArgumentException("Descriptor Type is invalid"),
													 ("Exception occured trying set the descriptors of the MBean"));
			}
		}
		...
		if (inDescriptorName == null)
		{ // thow RuntimeOperationsException - invalid descriptor type
			throw new RuntimeOperationsException(new IllegalArgumentException("Descriptor Name is invalid"),
												 ("Exception occured trying set the descriptors of the MBean"));
		}
		boolean found = false;
		if (inDescriptorType.equalsIgnoreCase(MMB))
		...
		} else if (inDescriptorType.equalsIgnoreCase(ATTR))
		{
			...
			//MBeanAttributeInfo[] attrList =  this.getAttributes();
			...
			for (int i=0; i < numAttrs; i++)
			{
				...
				{
				    found = true;
				    ((ModelMBeanAttributeInfo)attrList[i]).setDescriptor(inDescriptor);
					if (tracing())
					{
						trace("ModelMBeanInfoSupport.setDescriptor","setting descriptor to " + inDescriptor.toString());
						trace("ModelMBeanInfoSupport.setDescriptor","local: AttributeInfo descriptor is " + ((ModelMBeanAttributeInfo)attrList[i]).getDescriptor().toString());
						trace("ModelMBeanInfoSupport.setDescriptor","modelMBeanInfo: AttributeInfo descriptor is " + this.getDescriptor(inDescriptorName,"attribute").toString());
					}
				}
			}
		...
		} else if (inDescriptorType.equalsIgnoreCase(OPER))
		{
			...
			for (int i=0; i < numOpers; i++)
			{
				...
				{
				    found = true;
				    ((ModelMBeanOperationInfo)operList[i]).setDescriptor(inDescriptor);
				}
			}
		} else if (inDescriptorType.equalsIgnoreCase(CONS))
		{
			...
			for (int i=0; i < numCons; i++)
			{
				...
				{
				    found = true;
				    ((ModelMBeanConstructorInfo)consList[i]).setDescriptor(inDescriptor);
				}
			}
		} else if (inDescriptorType.equalsIgnoreCase(NOTF))
		{
			...
			for (int i=0; i < numNotifs; i++)
			{
				...
				{
				    found = true;
				    ((ModelMBeanNotificationInfo)notifList[i]).setDescriptor(inDescriptor);
				}
			}
		} else
		{
	...
	throws MBeanException, RuntimeOperationsException {
		...
		{ 		
		    throw new RuntimeOperationsException(new IllegalArgumentException("Attribute Name is null"),
							 ("Exception occured trying get the ModelMBeanAttributeInfo of the MBean"));
		}
		MBeanAttributeInfo[] attrList = modelMBeanAttributes;
		int numAttrs = 0;
		if (attrList != null) numAttrs = attrList.length;
	...
	throws MBeanException, RuntimeOperationsException {
		...
		{ 
		    throw new RuntimeOperationsException(new IllegalArgumentException("inName is null"),
							 ("Exception occured trying get the ModelMBeanOperationInfo of the MBean"));
		}
	...
	throws MBeanException, RuntimeOperationsException {
		...
		{ 
		    throw new RuntimeOperationsException(new IllegalArgumentException("Constructor name is null"),
							 ("Exception occured trying get the ModelMBeanConstructorInfo of the MBean"));
		}
	...
	throws MBeanException, RuntimeOperationsException {
		...
		{ 
		    throw new RuntimeOperationsException(new IllegalArgumentException("Notification name is null"),
							 ("Exception occured trying get the ModelMBeanNotificationInfo of the MBean"));
		}
	...
	*	persistPolicy=never,log=F,visiblity=1
	*/
	...
	*/
	private boolean isValidDescriptor(Descriptor inDesc)
	{
		...
		if (tracing()) trace("isValidDescriptor","Validating descriptor: " + inDesc.toString());
		if (inDesc == null)
		{
		...
		if ((((String)inDesc.getFieldValue("name")) == null))
		    // || (!((String)inDesc.getFieldValue("name")).equals(this.getClassName())))
		{
			badField="name";
			results = false;
		} else if (! ((String)inDesc.getFieldValue("descriptorType")).equalsIgnoreCase(MMB))
		{
			badField="descriptorType";
			results = false;
		} else if ((inDesc.getFieldValue("displayName")) == null)
		{
			inDesc.setField("displayName",this.getClassName());
		} else if ((inDesc.getFieldValue("persistPolicy")) == null)
		{
			inDesc.setField("persistPolicy","never");
		} else if ((inDesc.getFieldValue("log")) == null)
		{
			inDesc.setField("log","F");
		} else if ((inDesc.getFieldValue("visiblity")) == null)
		{
			inDesc.setField("visibility","1");
		}
		if (tracing()) trace("isValidDescriptor","returning " + results + ": Invalid field is " + badField);
		return results;
	}
