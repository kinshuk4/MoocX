...
public class ObjectName implements QueryExp, Serializable { 
    ...
    private final static class PropertyLengthCouple {
        ...
        public boolean equals(Object obj) {
            ...
            if (!(obj instanceof PropertyLengthCouple)) return false;

            PropertyLengthCouple couple = (PropertyLengthCouple) obj;
            if ((_key_length != couple._key_length) ||
                (_value_length != couple._value_length)) return false;
            else return true;
        }

        public int hashCode() { return (_key_length << 5) ^ _value_length; }
    ...
    private final static class Property {
        ...
        private static WeakHashMap _PropertyLengthCouples = 
	    new WeakHashMap(89);
        ...
        /**
         * Constructor. Every field is initialized once here
         */
        Property (int key_index, PropertyLengthCouple couple) {
            _key_index = key_index;        
            WeakReference tmp_ref;
            PropertyLengthCouple pl_couple;
            if (((tmp_ref = (WeakReference) 
		  _PropertyLengthCouples.get(couple)) != null) &&
                ((pl_couple = (PropertyLengthCouple) tmp_ref.get()) != null))
                _lengths = pl_couple;
            else {
                _lengths = new PropertyLengthCouple(couple._key_length, 
						    couple._value_length);
                // new length couple, record it.
                _PropertyLengthCouples.put(_lengths, 
					   new WeakReference(_lengths));
            }
        }
        ...
        String getKeyString(String name) {
            return name.substring(_key_index, _key_index + 
				  _lengths._key_length);
        }
        ...
        String getValueString(String name) {
            int in_begin = _key_index + _lengths._key_length + 1;
            int out_end = in_begin + _lengths._value_length;
    ...
    /**
     * The propertyList of built object name. Initialized lazylly.
	...
	if (name == null) throw new 
	    MalformedObjectNameException("Invalid name: cannot be null");
	...
	if (name.length() == 0) {
        ...
        char[] canonical_chars = new char[len]; // canonical form will be same length at most
        int cname_index = 0;
	int index = 0;
        char c, c1;
    ...
    domain_parsing:
        while (index < len) {
            switch ( c = name_chars[index]) {
                ...
                case '=' :
                    ...
                    if (i == len) throw new
                        MalformedObjectNameException("Domain part must be specified");
		    break;
                case '\n' :
        ...
        if (index == len) throw new
            MalformedObjectNameException("Key properties cannot be empty");
        ...
        while (index < len) {
            ...
            if (c == '*') {
                if (_property_pattern) throw new
                    MalformedObjectNameException("Cannot have several '*' characters in pattern properties");
                else {
                    _property_pattern = true;
                    if ((++index < len ) && (name_chars[index] != ',')) throw new
                        MalformedObjectNameException("Invalid character found after '*': end of name or ',' expected");
                    else if (index == len) {
                        if (property_index == 0) {
			...
			final String ichar = ((c1=='\n')?"\\n":""+c1);
                        throw new
                            MalformedObjectNameException("Invalid character '" + ichar + "' in key part part of property");
                    default: ;
                }
            if (in_index == len) throw new
                MalformedObjectNameException("Unterminated key property part");
            if (in_index == index) throw new
            ...
            quoted_value_parsing:
                while ((++in_index < len) && ((c1 = name_chars[in_index]) != '\"')) {
                    ...
                    if (c1 == '\\') {
                        if (++in_index == len) throw new
                            MalformedObjectNameException("Unterminated quoted value");
                        switch (c1 = name_chars[in_index]) {
                            ...
                            default :
                                throw new
                                    MalformedObjectNameException("Invalid escape sequence '\\" + c1 + "' in quoted value");
                        }
		    } else if (c1 == '\n') {
			throw new
			    MalformedObjectNameException("Newline in quoted value");
		    } else {
                        switch (c1) {
                            case '?'  :
                            case '*'  :
                                throw new
                                    MalformedObjectNameException("Invalid unescaped reserved character '" + c1 + "' in quoted value");
                            default:
                                break;
                        }
                    }
            ...
            if (in_index == len - 1) {
                if (quoted_value) throw new
                    MalformedObjectNameException("Invalid ending character `" + name_chars[in_index] + "'");
                else throw new
                    MalformedObjectNameException("Invalid ending comma");
            }
            else in_index++;

            // we got the key and value part, prepare a property for this
            prop_len_couple.setLengths(key_length, value_length);
            prop = new Property(key_index, prop_len_couple);
	...
	// Test if the name is empty
	    ...
	    MalformedObjectNameException("Domain part must be specified");
        if (props == null) throw new 
	    MalformedObjectNameException("Property list cannot be empty");

        // checks domain validity
	if (!isDomain(domain))
            throw new 
	    MalformedObjectNameException("Invalid domain: " + domain);
	...
	for (int i=0; e.hasMoreElements(); i++ ) {
            if (i > 0) sb.append(",");
	    String key = "";
	    try { key = (String)e.nextElement(); } catch (Exception x) {
		throw new MalformedObjectNameException("Invalid key `" + key + "'");
	    }
	    String value = "";
	    try { value = (String)props.get(key); } catch (Exception x) {
     ...
     */
    private void addProperty(Property prop, int index, HashMap keys_map, String key_name)
	throws MalformedObjectNameException {
    ...
    private void setCanonicalName(char[] specified_chars,
        ...
        if (_kp_array != _Empty_property_array) {
            ...
            for (int i = 0; i <= last_index; i++) {
                prop = _ca_array[i];
                prop_len = prop._lengths._key_length + prop._lengths._value_length + 1; // length of prop including '=' char
     ...
     *                            <li>&lt;canonicalName&gt; is the {@link String}
     *                                containing this {@link ObjectName}'s
     *                                canonical name</li>
     *                            <li>&lt;pattern&gt; is a boolean which is
     *                                <code>true</code> if this {@link ObjectName} 
     *                                contains a pattern</li> 
     *                            <li>&lt;propertyPattern&gt; is a boolean which is
     *                                <code>true</code> if this {@link ObjectName} 
     *                                contains a pattern in the list of properties</li> 
     *                          </ul>
     *               </li>
     *             </ul>
     */
	...
	if (table == null) throw new 
	    MalformedObjectNameException("Hashtable is null");
	if (table.isEmpty()) throw new
	    MalformedObjectNameException("Empty Hashtable");
        construct(domain, table);
    } 
    ...
    */
    ...
    private final Hashtable _getKeyPropertyList()  {
        synchronized (this) {
            if (_propertyList == null) {
                // build (lazy eval) the property list from the canonical properties array
	...
	final int last = len - 1;
        for (int i = 0; i < len; i++) {
            ...
            final int prop_len = prop._lengths._key_length + 
		prop._lengths._value_length +1;
