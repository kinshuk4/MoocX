...
// Revisit: This class needs serious reworking.
//
public class RequiredModelMBean 
     ...
     * An MBeanException wrapping a {@link ServiceNotFoundException} 
     * is thrown.
     **/
    public void load()
	throws MBeanException, RuntimeOperationsException, 
    ...
    /**
     * Persistence is not supported by the this Model MBean implementation.
     * An MBeanException wrapping a {@link ServiceNotFoundException} is 
     * thrown.
     **/
    public void store()
	throws MBeanException, RuntimeOperationsException, 
     ...
     * the given method will be tried on the managed ressource.
     * <P>
	...
	if ((opArgs != null) && (opArgs.length != 0)) {
	    ...
	    // intercept ModelMBean ifc calls and delegate to local 
	    // methods first
	    mmbArgs = opArgs;
		...
		try {
			...
			mmbClassFlag[i] = true;
			mmbSig[i] = loadClass(sig[i]);
		    }

		    if (tracing()) {
			String mmbArgOut;
			if (mmbArgs[i] == null)	mmbArgOut = "null";
			else mmbArgOut = mmbArgs[i].toString();
			trace("invoke(String, Object[], String[])",
			      "invoke method found a valid argument: " + 
			      mmbSig[i].toString() + " is " + mmbArgOut);
		    }
		    
		} catch (ClassNotFoundException cnfl) {
		    if (tracing()) {
			trace("invoke(String, Object[], String[])",
			      opName + " method " + opMethodName + 
			      "is not in RequiredModelMBean class:" + 
			      cnfl.getMessage());
			trace("invoke(String, Object[], String[])",
			      "The parameter class could not be found " + 
			      "in default class loader");
		    }
		    // we'll try this again with the class loader of 
		    // the target object
		    mmbClassFlag[i] = true;
		    mmbSig[i] = null;
	...
	if (( mmbOpHandle != null ) || mmbLocalInvoke) {
		...
		// do local invoke
		if (tracing()) 
		    trace("invoke(String, Object[], String[])",opName + 
			  " being invoked on ModelMBean");
		response = mmbOpHandle.invoke(this, mmbArgs);
		if (tracing()) 
		    trace("invoke(String, Object[], String[])",
			  "invoke returning " +
			  (response==null?"null":"non-null") +
			  " response from local invoke of " + opName);
		...
		if (mmbTargEx instanceof RuntimeException) {
		    ...
		    //    throw new MBeanException ((Error)mmbTargEx,
		    //  "Error thrown in RequiredModelMBean while " +
		...
		throw new RuntimeErrorException((Error)err, 
	...
	if (tracing()) 
	    trace("invoke(String, Object[], String[])",opName + 
		  " is not on RequiredModelMBean, " + 
		  "looking in descriptor for " + opMethodName);
	opInfo = modelMBeanInfo.getOperation(opMethodName);
	if (opInfo == null) {
	    throw new MBeanException(
		      new ServiceNotFoundException("operation " + opName + 
                      " execution not supported from mbeaninfo data"),
		      "An exception occured in RequiredModelMBean while "+
		      "trying to invoke an operation");
	}
	...
	if (opDescr == null) {	
	    ...
	    throw new MBeanException(
                      new ServiceNotFoundException("operation " + opName + 
		      " execution not supported from descriptor data"),
		      "An exception occured in RequiredModelMBean while "+
		      "trying to invoke an operation");
	} else {
		...
		if (tracing()) 
		    trace("invoke(String, Object[], String[])",
			  "No cached value returned for operation");

		// get target object
		Object currObj = 
		    (Object) opDescr.getFieldValue("targetObject");
		if (currObj != null) {
		    if (tracing()) 
			trace("invoke(String, Object[], String[])",
			      "Found target object in descriptor");
		    String currType = 
			(String) opDescr.getFieldValue("targetType");
		    if ((currType == null) || 
			(!currType.equalsIgnoreCase("objectReference"))) {
			throw new MBeanException(
			   new InvalidTargetObjectTypeException(currType),
			   "An exception occured while trying to invoke " +
			   "an operation on a descriptor provided target");
		    }
		    if (tracing()) 
			trace("invoke(String, Object[], String[]",
			      "target object is a valid type");
		} else {
		    if (managedResource != null) {
			if (tracing()) 
			    trace("invoke(String, Object[], String[])",
				  "managedResource for invoke found");
			currObj = managedResource;
		    } else {
			if (tracing()) 
			    trace("invoke(String, Object[], String[])",
				  "err managedResource for invoke is null");
			throw new MBeanException(
			      new ServiceNotFoundException(
				  "managedResource for invoke is null"), 
			      "An exception occured in RequiredModelMBean "+
			      "while trying to invoke an operation");
		    }
		}
		
		// recreate sig with class loader of target object

		// Have to use MethodDescriptor of mbeaninfo and strip off 
		// last token 
		//
		// note: this means that the opName is the fully qualified 
		//       class.method name
		...
		try {
			...
			throw new ReflectionException(
				  ...
				  "A valid class could not be found for " + 
				  opMethodName),
			      "Exception occured while trying to " +
			      "find class for method of invoke");
		    }
		} catch (ClassNotFoundException cnf) {
		    throw new ReflectionException(cnf, 
			      "The target object class " + opClassName + 
			      " could not be found");
		}
		...
		try {
		    if (tracing()) 
			trace("invoke(String, Object[], String[])", 
			      "setting signature with correct " + 
			      "class loaders for operation");
		...
		if (tracing()) 
		    trace("invoke(String, Object[], String[])",
			  "Looking for operations class " + opClassName + 
			  " method " + opMethodName);
		...
		try {
		    ...
		    if (opHandle == null) {
			throw new ReflectionException(
			      new Exception("null method handle"),
			      "Retrieved null method handle for method");
		    }
		    if (tracing()) 
			trace("invoke(String, Object[], String[])",
			      "Retrieved valid method handle for " + 
			      opClassName + "." + opMethodName);
		} catch (NoSuchMethodException nsm) {
		    throw new ReflectionException(nsm, "The method " + 
			  opClassName + "." + opMethodName + 
			  " could not be found");
		}
		...
		try {
		    ...
		    if (tracing()) 
			trace("invoke(String, Object[], String[])",
			      "invoke done for " + opName);

		    if ((response != null) && (opDescr != null)) {
			//if (tracing()) trace("response:" + response + 
			//      " opDescr:" + opDescr.toString());
			...
			if ((ctl != null) && !(ctl.equals("-1"))) {
				...
				trace("invoke(String,Object[],Object[])",
				      ...
				      opDescr.toString());
				trace("invoke(String,Object[],Object[])",
				      "OperationInfo descriptor is " + 
				      opInfo.getDescriptor().toString());
				trace("invoke(String,Object[],Object[])",
				      "OperationInfo descriptor is " + 
				      modelMBeanInfo.getDescriptor(opName,
						  "operation").toString());
			    }
			}
			if (tracing()) 
			    trace("invoke(String, Object[], String[])",
				  "invoke retrieved " + response.toString());
		    }
		} catch (RuntimeErrorException ree) {
		    throw new RuntimeOperationsException(ree, 
			  "RuntimeException occured in managed resource "+
			  "while trying to invoke operation " + opName);
		} catch (RuntimeException re) {
		    throw new RuntimeOperationsException(re, 
                          "RuntimeException occured in managed resource " +
		    ...
		    throw new ReflectionException(iae, 
			  "IllegalAccessException occured in managed " +
			  "resource while trying to invoke operation " + 
			  opName);
		} catch (Error err) {
		    throw new RuntimeErrorException((Error)err, 
			      "Error occured in managed resource while "+
			      "trying to invoke operation " + opName);
		} catch (InvocationTargetException ite)	{
		    Throwable mmbTargEx = ite.getTargetException();
		    if (mmbTargEx instanceof RuntimeException) {
			throw new RuntimeMBeanException(
				  (RuntimeException)mmbTargEx, 
				  "RuntimeException thrown in managed "+ 
				  "resource while trying to " +
				  "invoke operation " + opName);
		    } else if (mmbTargEx instanceof Error) {
			throw new RuntimeErrorException ((Error)mmbTargEx, 
                                  "Error thrown in managed resource while "+
				  "trying to invoke operation " + opName);
		    } else if (mmbTargEx instanceof ReflectionException) {
			throw (ReflectionException) mmbTargEx;
		    } else {
			throw new MBeanException ((Exception)mmbTargEx, 
                              "Exception thrown in managed resource while "+
			      "trying to invoke operation " + opName);
		    }
		} catch (Exception e) {
		    throw new ReflectionException(e, 
                              "Exception occured in managed resource "+
			      "while trying to invoke operation " + opName);
		}
	    }  // response ! cached

	    /* make sure response class matches type field */
	    /* I don't think we need to do this after all ..
	       if (tracing()) trace("invoke checking return type");
	       String respType = opInfo.getReturnType();
	       String responseClass = "void";
	       if ( !respType.equals("void") && !respType.equals("null") )
	       {
	       responseClass = (response.getClass()).getName();
	       }

	       if ( ! respType.equals(responseClass) )
	       {
	       if (tracing()) trace("invoke","response class missmatch, "+
	                       "returning null");
     ...
     * the MBean server and re-thrown as an {@link javax.management.MBeanRegistrationException}
     * or a {@link RuntimeMBeanException}.
     */
