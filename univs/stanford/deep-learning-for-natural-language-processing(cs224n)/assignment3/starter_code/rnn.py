...
class RNN:
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        if node.isLeaf:
            ...
            node.hActs1 = np.dot(self.W, np.hstack([node.left.hActs1, node.right.hActs1])) + self.b # (wvecDim, 2*wvecDim) * (2*wvecDim, )
            # ReLu
            node.hActs1[node.hActs1 < 0] = 0

        node.probs = softmax(np.dot(self.Ws, node.hActs1) + self.bs)
