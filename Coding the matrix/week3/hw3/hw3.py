...
def dot_product_mat_vec_mult(M, v):
    ...
    return Vec(M.D[0], {i: RowDict[i] * v for i in M.D[0]})

# # Problem 14
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    ColDict = mat2coldict(M)
    return Vec(M.D[1], {i: v * ColDict[i] for i in M.D[1]})
